    ...
    <div class="relative flex flex-col h-screen w-fit fixed justify-between py-4">
      ...
      <div class="relative flex flex-col space-y-2" @mouseleave="activeTopIndex = null">
        ...
        <div class="flex flex-col items-center ">
          <NavButton v-for="(button, index) in buttons" :key="index" v-bind="button"
            @mouseenter="activeTopIndex = index" :isActive="activeTopIndex === index" />
        </div>
      </div>

      <!-- Sección inferior -->
      <div class="flex flex-col gap-y-4 pb-4" @mouseleave="activeBottomIndex = null">
        <div class="absolute left-0 w-1  bg-white transition-all duration-300 ease-in-out" :style="bottomLineStyle">
        </div>
        <div class="flex flex-col items-center ">
