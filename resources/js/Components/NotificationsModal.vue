...
<template>
  <div v-if="isOpen"
    ...
    <div class="flex items-center justify-between p-3 border-b border-gray-700">
      ...
      <div v-if="props.notifications.length > 0" class="flex items-center gap-2">
        <button @click="markAllAsRead"
          class="text-xs cursor-pointer text-gray-400 hover:text-white transition-colors duration-200">
          Mark all as read
        </button>
      </div>
    </div>
    ...
    <div class="max-h-96 overflow-y-auto custom-scrollbar">
      ...
      <div v-else>
        <div v-for="(notification, index) in props.notifications" :key="index"
          class="p-3 border-b border-gray-600 hover:bg-gray-800 dark:hover:bg-gray-100 dark:bg-gray-200 transition-colors duration-200 cursor-pointer"
          :class="{ 'bg-gray-700 bg-opacity-40': !notification.is_read }" @click="markAsRead(notification)">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center justify-between">
                <p class="font-medium text-sm"
                  :class="{ 'text-white dark:text-black': !notification.is_read, 'text-gray-300 dark:text-gray-700': notification.is_read }">
                  {{ notification.title }}
                </p>
    ...
    <div class="p-1 border-t border-gray-700">
      <button
        class="w-full py-1 text-sm cursor-pointer text-center text-gray-300 dark:text-gray-500 hover:text-white dark:hover:bg-gray-200 dark:hover:text-gray-700 transition-colors duration-200 rounded-lg hover:bg-gray-800"
