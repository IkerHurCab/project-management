  ...
  <div class="max-w-7xl">
    <div v-if="!openSingleDoc">
      <div class="mb-6 flex w-full gap-x-4">
        <InputWithIcon icon="search" v-model="searchQuery" @input="filterDocumentations"
          placeholder="Search documentation..." class="flex-grow" type="text" />
        <StandardButton v-if="isUserInProject" @click="createDoc = true; openSingleDoc = true" class="py-2">
          + Add Documentation
        </StandardButton>
      </div>
      ...
      <div class="flex flex-row gap-6">
        <div v-for="doc in filteredDocumentations" :key="doc.id" @click="selectDocumentation(doc)"
          class="bg-gray-950 dark:bg-white dark:border-none dark:shadow-xl cursor-pointer rounded-lg p-6 border border-gray-700 lg:w-1/3 md:1/2 sm:w-full hover:bg-gray-900 dark:hover:bg-gray-200 group transition duration-300">
          <h3 class="text-lg font-semibold text-white dark:text-black mb-2 group-hover:underline">{{ doc.title }}</h3>
    ...
    <div v-else class="p-6 w-full">
      <div class="flex justify-between">
        <div @click="openSingleDoc = false; isEdit = false"
          class="flex hover:font-semibold flex-row gap-x-4 cursor-pointer">
          <box-icon name='arrow-back' :color="checkDarkMode() ? 'black' : 'white'"></box-icon>
      ...
      <div class="mt-5">
        ...
        <div v-if="isEdit">
          <DocumentationModal :documentation="selectedDocumentation" :isEdit="true" :project="project"
